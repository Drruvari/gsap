...
interface EditorState {
  ...
  updateElementsLayout: (updates: Array<{ id: string; x: number; y: number }>, commit?: boolean) => void;
...
export const useEditorStore = create<EditorState>()(persist((set, _get) => ({
  elements: [],
  selectedId: null,
  selectedIds: [],
  isPlaying: false,
  ...
  updateElementLayout: (id, layout, commit = true) => set((state) => ({
    ...(commit
      ? { historyPast: [...state.historyPast, createSnapshot(state)].slice(-MAX_HISTORY), historyFuture: [] }
      : {}),
  ...
  updateElementSize: (id, size, commit = true) => set((state) => ({
    ...(commit
      ? { historyPast: [...state.historyPast, createSnapshot(state)].slice(-MAX_HISTORY), historyFuture: [] }
      : {}),
    elements: state.elements.map((el) =>
  ...
  updateElementsLayout: (updates, commit = true) => set((state) => {
  ...
  updateElementAnimation: (id, props) => set((state) => ({
    historyPast: [...state.historyPast, createSnapshot(state)].slice(-MAX_HISTORY),
    historyFuture: [],
    elements: state.elements.map((el) =>
  ...
  selectElement: (id, additive = false) => set((state) => {
    ...
    if (additive) {
      ...
      return {
        historyPast: [...state.historyPast, createSnapshot(state)].slice(-MAX_HISTORY),
        historyFuture: [],
        selectedId: nextSelectedIds.length ? nextSelectedIds[nextSelectedIds.length - 1] : null,
  ...
  deleteElement: (id) => set((state) => {
    ...
    const nextSelectedId = state.selectedId === id
  ...
  alignSelected: (mode) => set((state) => {
    ...
    const updates = selected.map((el) => {
      ...
      if (mode === 'center') return { id: el.id, x: minX + (maxX - minX) / 2 - el.size.w / 2, y: el.layout.y };
  ...
  distributeSelected: (axis) => set((state) => {
    ...
    if (axis === 'x') {
      ...
      const space = (max.layout.x + max.size.w - min.layout.x - totalSize) / (selected.length - 1);
    ...
    const space = (max.layout.y + max.size.h - min.layout.y - totalSize) / (selected.length - 1);
  ...
  merge: (persisted, current) => {
    ...
    state.elements = (state.elements ?? []).map((el) => {
      ...
      const normalizedSize =
        currentSize.w < 80 || currentSize.h < 80 ? fallback : currentSize;
      return {
        ...el,
        size: normalizedSize,
